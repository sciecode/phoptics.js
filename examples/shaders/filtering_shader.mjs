...
fn D_GGX(cosNH : f32, alpha : f32) -> f32 {
  let a = cosNH * alpha;
  let k = alpha / (1.0 - cosNH * cosNH + a * a);
  return k * k * R_PI;
}

fn GGX(xi : vec2f) -> Sample {
  let alpha = info.roughness * info.roughness;
  
  var ggx : Sample;
  ggx.cosT = saturate(sqrt((1. - xi.y) / (1.0 + (alpha * alpha - 1.0) * xi.y)));
  ggx.sinT = sqrt(1. - ggx.cosT * ggx.cosT);
  ggx.phi = 2.0 * PI * xi.x;
  ggx.pdf = D_GGX(ggx.cosT, alpha) / 4.;

  return ggx;
}

fn importance_sample(idx : i32, N : vec3f) -> vec4f {
...
fn apply_filter(N : vec3f) -> vec3f {
  ...
  let count : i32 = i32(info.sample_count);
  for (var i = 0; i < count; i++) {
    let sample = importance_sample(i, N);
